...
- The `second method <#method-2-identify-the-text-language-i-e-multi-classification-textcat-from-nltk>`_ uses the
  ...
  `method 1 <#method-1-detect-only-if-it-is-english-or-not-i-e-binary-classification-nltk-english-corpus>`_ which only 
  tells you if the text is English or not.
